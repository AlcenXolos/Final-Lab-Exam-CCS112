...
const MedicalRecordManagement = ({ currentUser }) => {
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="mb-3">
        {!addingNew && (
          <button className="btn btn-success" onClick={() => setAddingNew(true)}>Add New Record</button>
        )}
        {addingNew && (
          <div>
            ...
            <button className="btn btn-primary mr-2" onClick={handleAddNew}>Save</button>
            <button className="btn btn-danger" onClick={() => setAddingNew(false)}>Cancel</button>
          </div>
        )}
      </div>
    </div>
